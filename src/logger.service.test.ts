  ...
  describe('with injected adapter', () => {
    ...
    beforeEach(async () => {
      ...
      const module = await Test.createTestingModule({
        providers: [
          LoggerService,
          { provide: LOGGER_ADAPTER, useValue: mockAdapter },
        ],
      }).compile()
    ...
    it('should delegate info to adapter', () => {
      ...
      expect(adapter.info).toHaveBeenCalledWith('hello', { key: 'value' })
    })
    ...
    it('should delegate debug to adapter', () => {
      ...
      expect(adapter.debug).toHaveBeenCalledWith('debug msg', { foo: 'bar' })
    })

    it('should delegate warn to adapter', () => {
      logger.warn('watch out', { remaining: 5 })

      expect(adapter.warn).toHaveBeenCalledWith('watch out', { remaining: 5 })
    })

    it('should delegate error to adapter', () => {
      logger.error('broken', { code: 'ERR' })

      expect(adapter.error).toHaveBeenCalledWith('broken', { code: 'ERR' })
    })
